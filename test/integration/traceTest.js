...
QUnit.test("Simple strong trace equivalence", function ( assert ) {
    ...
    assert.ok(Equivalence.isTraceIncluded(strongSuccGen, strongSuccGen, processP.id, processQ.id, graph), "P should be trace included in Q");
    assert.ok(Equivalence.isTraceIncluded(strongSuccGen, strongSuccGen, processQ.id, processP.id, graph), "Q should be trace included in P");
});
...
QUnit.test("Simple one way inclusion", function ( assert ) {
    var graph = CCSParser.parse("P = a.0; Q = a.0 + b.0;", {ccs: CCS}),
        ...
        processQ = graph.processByName("Q");
    assert.ok(Equivalence.isTraceIncluded(strongSuccGen, strongSuccGen, processP.id, processQ.id, graph), "P should be trace included in Q");
    assert.ok(!Equivalence.isTraceIncluded(strongSuccGen, strongSuccGen, processQ.id, processP.id, graph), "Q should not be trace included in P");
});
...
QUnit.test("Simple weak trace inclusion", function ( assert ) {
    var graph = CCSParser.parse("P = a.tau.0; Q = a.0;", {ccs: CCS}),
        ...
        processP = graph.processByName("P"),
        processQ = graph.processByName("Q");
    assert.ok(!Equivalence.isTraceIncluded(strongSuccGen, strongSuccGen, processP.id, processQ.id, graph), "P should not be strongly trace included in Q");
    assert.ok(Equivalence.isTraceIncluded(weakSuccGen, weakSuccGen, processP.id, processQ.id, graph), "P should be weakly trace included in Q");
});
