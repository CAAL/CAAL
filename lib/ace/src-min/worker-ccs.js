...
CCSParser = (function() {
  ...
  function parse(input) {
    ...
    function peg$parseWhitespaceNewline() {
      var s0;
...
(function (CCS) {
    ...
    var LabelSet = (function () {
        ...
        LabelSet.prototype.addLabels = function (labels) {
        ...
        LabelSet.prototype.removeLabels = function (labels) {
            for (var i = 0, max = labels.length; i < max; i++) {
                var label = labels[i];
        ...
        LabelSet.prototype.add = function (labels) {
        ...
        LabelSet.prototype.union = function (set) {
            return this.add(set.labels);
        };

        LabelSet.prototype.difference = function (set) {
    ...
    var Transition = (function () {
        ...
        Transition.prototype.hash = function () {
    ...
    var TransitionSet = (function () {
        ...
        TransitionSet.prototype.mergeInto = function (tSet) {
        ...
        TransitionSet.prototype.hashsetArray = function (hash) {
        ...
        TransitionSet.prototype.addTransition = function (transition) {
            var hash = transition.hash(), existingHashset = this.hashsetArray(hash);
            for (var i = 0; i < existingHashset.length; i++) {
                if (existingHashset[i].equals(transition))
                    return;
            }
        ...
        TransitionSet.prototype.removeInPlace = function (labels) {
            for (var hashKey in this.transitions) {
                this.transitions[hashKey] = this.transitions[hashKey].filter(function (t) {
                    return !labels.contains(t.action.label);
                });
        ...
        TransitionSet.prototype.relabelInPlace = function (relabels) {
            for (var hashKey in this.transitions) {
                this.transitions[hashKey].forEach(function (t) {
        ...
        SuccessorGenerator.prototype.dispatchCompositionProcess = function (process) {
            ...
            if (!transitionSet) {
                ...
                leftSet.forEach(function (leftTransition) {
                    transitionSet.addTransition(new Transition(leftTransition.action.clone(), _this.graph.newCompositionProcess(leftTransition.targetProcess, process.rightProcess)));

                    rightSet.forEach(function (rightTransition) {
                        transitionSet.addTransition(new Transition(rightTransition.action.clone(), _this.graph.newCompositionProcess(process.leftProcess, rightTransition.targetProcess)));
                        if (leftTransition.action.label === rightTransition.action.label && leftTransition.action.isComplement !== rightTransition.action.isComplement) {
                            transitionSet.addTransition(new Transition(new Action("tau", false), _this.graph.newCompositionProcess(leftTransition.targetProcess, rightTransition.targetProcess)));
                        }
                    });
                });
            }
